...
"wqvdldrZOj9LCYJ8mer+L0DR4a0UDQAA";
    ...
    static get_chip_revision = async (loader) => {
        let word3 = await this.read_efuse(loader, 3);
        let word5 = await this.read_efuse(loader, 5);
        let apb_ctl_date = await loader.read_reg({addr: this.DR_REG_SYSCON_BASE + 0x7C});
    ...
    static get_chip_description = async (loader) => {
        let chip_desc = ["ESP32-D0WDQ6", "ESP32-D0WD", "ESP32-D2WD", "", "ESP32-U4WDH", "ESP32-PICO-D4", "ESP32-PICO-V3-02"];
        let chip_name = "";
        let pkg_version = await this.get_pkg_version(loader);
        let chip_revision = await this.get_chip_revision(loader);
        let rev3 = (chip_revision == 3);
        let single_core = await this.read_efuse(loader, 3) & (1 << 0);
    ...
    static get_chip_features = async (loader) => {
        ...
        let word4 = await this.read_efuse(loader, 4);
        ...
        let word6 = await this.read_efuse(loader, 6);
        let coding_scheme = word6 & 0x3;
        let coding_scheme_arr = ["None", "3/4", "Repeat (UNSUPPORTED)", "Invalid"];
    ...
    static get_crystal_freq = async (loader) => {
        let uart_div = await loader.read_reg({addr: this.UART_CLKDIV_REG}) & this.UART_CLKDIV_MASK;
        let ets_xtal = (loader.transport.baudrate * uart_div) / 1000000 / this.XTAL_CLK_DIVIDER;
        let norm_xtal;
        if (ets_xtal > 33) {
            norm_xtal = 40;
        } else {
...
"PhaAOeuxgvFPCyIcXl5oXscBJKBdOTgbirB0QdA+obr/HyfMWQisDwAA";
    ...
    static get_pkg_version = async (loader) => {
        let num_word = 3;
        let block1_addr = this.EFUSE_BASE + 0x044;
        let addr = block1_addr + (4 * num_word);
        let word3 = await loader.read_reg({addr: addr});
        let pkg_version = (word3 >> 21) & 0x0F;
        return pkg_version;
    }

    static get_chip_description = async (loader) => {
        let chip_desc = [ "ESP32-S2", "ESP32-S2FH16", "ESP32-S2FH32"];
        let pkg_ver = await this.get_pkg_version(loader);
    ...
    static get_chip_features = async (loader) => {
        ...
        let num_word = 4;
        let block2_addr = this.EFUSE_BASE + 0x05C;
        let addr = block2_addr + (4 * num_word);
        let word4 = await loader.read_reg({addr: addr});
        let block2_ver = (word4 >> 4) & 0x07;
...
"J279YCuxbUpfYBg26DI+z0jM/CCTyLqft1P4h/4HprTSLAwNAAA=";
    ...
    static get_chip_revision = async (loader) => {
        ...
        let pos = 18;
        let addr = block1_addr + (4 * num_word);
        let ret = (await loader.read_reg({addr: addr}) & (0x7 << pos)) >> pos;
        return ret;
    }
    ...
    static get_chip_description = async (loader) => {
        ...
        let chip_rev = await this.get_chip_revision(loader);
...
class ESPLoader {
    ...
    _appendArray(arr1, arr2) {
        let c = new Uint8Array(arr1.length + arr2.length);
        c.set(arr1, 0);
    ...
    command = async ({op=null, data=[], chk=0, wait_response=true, timeout=3000} = {})  => {
        ...
        if (op != null) {
            let pkt = new Uint8Array(8 + data.length);
            pkt[0] = 0x00;
            pkt[1] = op;
        ...
        if (wait_response) {
            try {
                let p = await this.transport.read({timeout: timeout});
                const resp = p[0];
                const op_ret = p[1];
    ...
    read_reg = async({addr, timeout = 3000} = {}) => {
        let val, data;
        let pkt = this._int_to_bytearray(addr);
    ...
    connect = async ({mode='default_reset', attempts=7, detecting=false} = {}) => {
        ...
        if (!detecting) {
            let chip_magic_value = await this.read_reg({addr:0x40001000});
            console.log("Chip Magic " + chip_magic_value);
            let chips = [ESP8266ROM, ESP32ROM, ESP32S2ROM, ESP32S3BETA2ROM, ESP32C3ROM];
    ...
    check_command = async ({op_description="", op=null, data=[], chk=0, timeout=3000} = {}) => {
        ...
        let resp = await this.command({op:op, data:data, chk:chk, timeout:timeout});
        if (resp[1].length > 4) {
            return resp[1];
        } else {
            return resp[0];
    ...
    mem_block = async (buffer, seq) => {
        ...
        let checksum = this.checksum(buffer);
    ...
    mem_finish = async (entrypoint) => {
        let is_entry = (entrypoint === 0) ? 1 : 0;
        let pkt = this._appendArray(this._int_to_bytearray(is_entry), this._int_to_bytearray(entrypoint));
    ...
    timeout_per_mb = function(seconds_per_mb, size_bytes) {
        let result = seconds_per_mb * (size_bytes / 1000000);
        if (result < 3000) {
            return 3000;
        } else {
            return result;
    ...
    flash_begin = async (size, offset) => {
        ...
        let erase_size = this.chip.get_erase_size(offset, size);
    ...
    flash_defl_begin = async (size, compsize, offset) => {
        let num_blocks = Math.floor((compsize + this.FLASH_WRITE_SIZE - 1) / this.FLASH_WRITE_SIZE);
        let erase_blocks = Math.floor((size + this.FLASH_WRITE_SIZE - 1) / this.FLASH_WRITE_SIZE);
    ...
    flash_block = async (data, seq, timeout) => {
        ...
        let checksum = this.checksum(data);
    ...
    flash_finish = async ({reboot = false } = {}) => {
        let val = reboot ? 0 : 1;
        let pkt = this._int_to_bytearray(val);
    ...
    run_spiflash_command = async (spiflash_command, data, read_bits) => {
        ...
        let base = this.chip.SPI_REG_BASE;
        let SPI_CMD_REG = base + 0x00;
        let SPI_USR_REG       = base + this.chip.SPI_USR_OFFS;
        let SPI_USR1_REG      = base + this.chip.SPI_USR1_OFFS;
        let SPI_USR2_REG      = base + this.chip.SPI_USR2_OFFS;
        let SPI_W0_REG        = base + this.chip.SPI_W0_OFFS;

        let set_data_lengths;
        if (this.chip.SPI_MOSI_DLEN_OFFS != null) {
            set_data_lengths = async(mosi_bits, miso_bits) => {
                let SPI_MOSI_DLEN_REG = base + this.chip.SPI_MOSI_DLEN_OFFS;
                let SPI_MISO_DLEN_REG = base + this.chip.SPI_MISO_DLEN_OFFS;
                if (mosi_bits > 0) {
            ...
            set_data_lengths = async(mosi_bits, miso_bits) => {
                ...
                let val = (miso_mask << SPI_MISO_BITLEN_S) | (mosi_mask << SPI_MOSI_BITLEN_S);
        ...
        let old_spi_usr = await this.read_reg({addr:SPI_USR_REG});
        let old_spi_usr2 = await this.read_reg({addr:SPI_USR2_REG});
        ...
        if (data_bits == 0) {
            ...
            if (data.length % 4 != 0) {
                let padding = new Uint8Array(data.length % 4);
        ...
        let stat = await this.read_reg({addr:SPI_W0_REG});
    ...
    run_stub = async () => {
        ...
        let chardata = decoded.split('').map(function(x){return x.charCodeAt(0);});
        let bindata = new Uint8Array(chardata);

        let text = pako.inflate(bindata);

        decoded = atob(this.chip.ROM_DATA);
        chardata = decoded.split('').map(function(x){return x.charCodeAt(0);});
        let data = new Uint8Array(chardata);
        ...
        for (i = 0; i < blocks; i++) {
            let from_offs = i * this.ESP_RAM_BLOCK;
    ...
    main_fn = async () => {
        ...
        let chip = await this.chip.get_chip_description(this);
    ...
    _update_image_flash_params = function(image, address, flash_size, flash_mode, flash_freq) {
        ...
        let flash_params = (a_flash_mode << 8) | (a_flash_freq + a_flash_size);
    ...
    flash_id = async() => {
        ...
        let flashid = await this.read_flash_id();
        this.log("Manufacturer: " + (flashid & 0xff).toString(16));
        let flid_lowbyte = (flashid >> 16) & 0xff;
...
/**
 * Modified by masuidrive and Apache 2.0 license.
 * 
 * replace "var" to "let" and some "import" statements.
 * 
 * The code is based on Copyright (c) 2021 Espressif Systems (Shanghai) Co. Ltd. It is licensed under Apache 2.0 license, as described in https://github.com/espressif/esptool-js
 */